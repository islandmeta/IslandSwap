...
// File: contracts/interfaces/IIslandSwapLottery.sol

pragma solidity ^0.8.4;

interface IIslandSwapLottery {
    /**
     ...
     */
    function closeLottery(uint256 _lotteryId,uint256 _externalRandomNumber) external;

    /**
     * @notice Draw the final number, calculate reward in Island per group, and make lottery claimable
     * @param _lotteryId: lottery id
     ...
     * @notice Inject funds
     * @param _lotteryId: lottery id
     * @param _amount: amount to inject in Island token
     * @dev Callable by operator
     */
...
// File: contracts/IslandSwapLottery.sol

pragma solidity ^0.8.4;
pragma abicoder v2;

/** @title IslandSwap Lottery.
...
contract IslandSwapLottery is ReentrancyGuard, IIslandSwapLottery, Ownable {
    ...
    uint256 private winningNumber=1*10**6;
    ...
    uint256 public maxPriceTicketInIsland = 50 ether;
    uint256 public minPriceTicketInIsland = 0.005 ether;

    uint256 public pendingInjectionNextLottery;
    uint256 public lastTimestamp;

    uint256 public constant MIN_DISCOUNT_DIVISOR = 300;
    uint256 public constant MIN_LENGTH_LOTTERY = 1 hours; // 1 hours
    uint256 public constant MAX_LENGTH_LOTTERY = 3 days; // 3 days
    uint256 public constant MAX_TREASURY_FEE = 3000; // 30%

    IERC20 public islandToken;
    ...
    struct Lottery {
        ...
        uint256 amountCollectedInIsland;
        uint32 finalNumber;
    }
    ...
    /**
     * @notice Constructor
     * @param _islandTokenAddress: address of the Island token Island标记的地址
     */
    constructor(address _islandTokenAddress) {
        islandToken = IERC20(_islandTokenAddress);
    ...
    function claimTickets(
        ...
        // Initializes the rewardInIslandToTransfer
        uint256 rewardInIslandToTransfer;
        ...
        for (uint256 i = 0; i < _ticketIds.length; i++) {
            ...
            rewardInIslandToTransfer += rewardForTicketId;
        }

        // Transfer money to msg.sender
        islandToken.safeTransfer(msg.sender, rewardInIslandToTransfer);

        emit TicketsClaim(msg.sender, rewardInIslandToTransfer, _lotteryId, _ticketIds.length);
    }
     ...
     * @dev Callable by operator
     */
    function closeLottery(uint256 _lotteryId,uint256 _externalRandomNumber) external override  onlyOperator nonReentrant {
        require(_lotteries[_lotteryId].status == Status.Open, "Lottery not open");
        require(block.timestamp > _lotteries[_lotteryId].endTime, "Lottery not over");
        _lotteries[_lotteryId].firstTicketIdNextLottery = currentTicketId;
        lastTimestamp=block.timestamp;
        // Request a random number from the generator based on a seed
        generatorRandomNumber(_externalRandomNumber);
 ...
 // add externalRandomNumber to prevent node validators exploiting
    function generatorRandomNumber(uint256 _externalRandomNumber) internal onlyOperator {
        ...
        // // 2
        _structHash = keccak256(abi.encode(_blockhash,_lotteries[currentTicketId].amountCollectedInIsland,gasleft,_externalRandomNumber));
        _randomNumber  = uint256(_structHash);
        assembly {_randomNumber := add(mod(_randomNumber, _maxNumber),1)}
        winningNumber=winningNumber.add(_randomNumber*10**4);

        // // 3
        _structHash = keccak256(abi.encode(_blockhash,lastTimestamp,gasleft,_externalRandomNumber));
        _randomNumber  = uint256(_structHash);
        assembly {_randomNumber := add(mod(_randomNumber, _maxNumber),1)}
        winningNumber=winningNumber.add(_randomNumber*10**3);

        // // 4
        _structHash = keccak256(abi.encode(_blockhash,gasleft,_externalRandomNumber,lastTimestamp));
        _randomNumber  = uint256(_structHash);
        assembly {_randomNumber := add(mod(_randomNumber, _maxNumber),1)}
        winningNumber=winningNumber.add(_randomNumber*10**2);

        // // 5
        _structHash = keccak256(abi.encode(_blockhash,gasleft,totalAddresses,_lotteries[currentTicketId].amountCollectedInIsland));
        _randomNumber  = uint256(_structHash);
        assembly {_randomNumber := add(mod(_randomNumber, _maxNumber),1)}
        winningNumber=winningNumber.add(_randomNumber*10);

        // // 6
        _structHash = keccak256(abi.encode(_blockhash,gasleft,_lotteries[currentTicketId].amountCollectedInIsland,_externalRandomNumber));
        _randomNumber  = uint256(_structHash);
        assembly {_randomNumber := add(mod(_randomNumber, _maxNumber),1)}
        winningNumber=winningNumber.add(_randomNumber);
        ...
        nonReentrant
    {
        ...
        uint32 finalNumber = uint32(winningNumber);
        ...
        amountToWithdrawToTreasury += (_lotteries[_lotteryId].amountCollectedInIsland - amountToShareToWinners);

        // Transfer Island to treasury address
        islandToken.safeTransfer(treasuryAddress, amountToWithdrawToTreasury);
    ...
    function injectFunds(uint256 _lotteryId, uint256 _amount) external override onlyOwnerOrInjector {
        ...
        islandToken.safeTransferFrom(address(msg.sender), address(this), _amount);
        _lotteries[_lotteryId].amountCollectedInIsland += _amount;
    ...
    function startLottery(
        ...
        require(
            (_priceTicketInIsland >= minPriceTicketInIsland) && (_priceTicketInIsland <= maxPriceTicketInIsland),
            "Outside of limits"
        );
    ...
    function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount) external onlyOwner {
        require(_tokenAddress != address(islandToken), "Cannot be Island token");
        ...
        onlyOwner
    {
        ...
        minPriceTicketInIsland = _minPriceTicketInIsland;
        maxPriceTicketInIsland = _maxPriceTicketInIsland;
    }
    ...
    */
    ...
    function _calculateRewardsForTicketId(
        ...
        if (transformedWinningNumber == transformedUserNumber) {
            return _lotteries[_lotteryId].islandPerBracket[_bracket];
        } else {
            return 0;
        }
    }
