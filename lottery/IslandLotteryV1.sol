...
contract Lottery is LotteryOwnable, Initializable {
    ...
    constructor(address _metaData,address _islandNftFactory,address _vaultAddress) public {
        islandNFTMetaData=IislandNFTMetaData(_metaData);
        islandNFTFactory=IislandNFTFactory(_islandNftFactory);
    ...
    function drawingAndReset(uint256 _externalRandomNumber) external onlyAdmin{
        enterDrawingPhase();
    ...
    function reset() public onlyAdmin {
        ...
        islToken.transfer(destroyAddress,getTotalRewards(issueIndex-1).mul(burnDivisor).div(100));
        islToken.transfer(vaultAddress,getTotalRewards(issueIndex-1).mul(vaultShare).div(100));
    ...
    function enterDrawingPhase() public onlyAdmin {
        require(!drawed(), 'drawed');
        drawingPhase = true;
    }
    ...
    function buy(uint256 _price, uint8[4] memory _numbers) external {
        ...
        islToken.safeTransferFrom(address(msg.sender), address(this), _price);
    ...
    function  multiBuy(uint256 _price, uint8[4][] memory _numbers) external {
        ...
        islToken.safeTransferFrom(address(msg.sender), address(this), totalPrice);
    ...
    function multiClaim(uint256[] memory _tickets) external {
        ...
        if(totalReward>0) {
            islToken.safeTransfer(address(msg.sender),totalReward);
        }
    ...
    function setDestroyAddress(address _destroyAddress) external onlyAdmin {
        destroyAddress = _destroyAddress;
    }
