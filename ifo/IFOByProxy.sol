...
import "../libraries/proxy/Initializable.sol";
...
contract IFOByProxy is ReentrancyGuard, Initializable {
  ...
  function deposit(uint256 _amount) public {
    ...
    receiveToken.transferFrom(address(msg.sender), address(this), _amount);
  ...
  function harvest() public nonReentrant {
    ...
    if (refundingTokenAmount > 0) {
      receiveToken.transfer(address(msg.sender), refundingTokenAmount);
    }
  ...
  function finalWithdrawreceiveToken(uint256 _lpAmount) public onlyAdmin {
    require(block.number > endBlock,"It's not time to withdraw");
    require (_lpAmount <= receiveToken.balanceOf(address(this)), 'not enough token 0');
    require(totalLpAmount.add(_lpAmount) <= raisingAmount,'over the raisingAmount');
    if(_lpAmount > 0) {
        receiveToken.transfer(address(msg.sender), _lpAmount);
    }
